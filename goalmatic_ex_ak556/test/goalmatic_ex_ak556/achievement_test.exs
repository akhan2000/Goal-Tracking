defmodule GoalmaticExAk556.AchievementTest do
  use GoalmaticExAk556.DataCase
  ...
  describe "challenges" do
    ...
    test "create_challenge/1 with valid data creates a challenge" do
      valid_attrs = %{description: "some description", ends_at: ~U[2023-03-26 01:08:00Z], goal: 42, name: "some name", units: "some units"}

      assert {:ok, %Challenge{} = challenge} = Achievement.create_challenge(valid_attrs)
      assert challenge.description == "some description"
      assert challenge.ends_at == ~U[2023-03-26 01:08:00Z]
    ...
    test "update_challenge/2 with valid data updates the challenge" do
      challenge = challenge_fixture()
      update_attrs = %{description: "some updated description", ends_at: ~U[2023-03-27 01:08:00Z], goal: 43, name: "some updated name", units: "some updated units"}
      ...
      assert challenge.ends_at == ~U[2023-03-27 01:08:00Z]
